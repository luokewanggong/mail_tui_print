#!/bin/bash
#!/bin/bash
# MailTUI install.sh 一键部署&启动脚本
SCRIPT_VER="install‑v1.2"
...
check_root(){
    if [[ $EUID -ne 0 ]]; then
        echo -e "\033[33m[提示]建议使用root执行，用于安装系统依赖\033[0m"
    fi
}

install_deps(){
    echo "[1/4] 安装系统依赖 curl mpack"
    apt update -y
    apt install -y curl mpack
}

build_mail_tui(){
echo "[2/4] 生成 mail_tui.sh"
cat > mail_tui.sh <<'MAIN_EOF'
#!/bin/bash
# MailTUI 邮件附件自动下载工具
SCRIPT_VER="mailtui‑v2.1‑attach‑nocups"
args="$@"
...
echo "[3/4] 设置执行权限"
chmod +x mail_tui.sh
mkdir -p ./mail_download ./mail_download/attachments
}

run_app(){
echo "[4/4] 启动 MailTUI 交互界面"
echo ""
exec ./mail_tui.sh
}
